...
with st.sidebar:
    ...
    st.markdown(
        f'<div style="background:rgba(20,20,35,0.7);border-left:4px solid {_bot_color};'
        f'padding:8px 12px;border-radius:6px;margin:4px 0 10px 0;">'
...
if _app["auto_trade"] and latest_price:
    ...
    if model and signal_result:
        ...
        _target  = (1 if _prob >= long_threshold else (-1 if _prob <= short_threshold else 0))
        _cur_side = oanda_pos["side"]
...
def _prediction_feed():
    ...
    try:
        _lt = float(_app["active_params"]["long_threshold"])
        _st = float(_app["active_params"]["short_threshold"])
...
def _tick_feed():
    ...
    if api:
        ...
        _app["price_history"] = _app["price_history"][-500:]
    ...
    if _app["price_history"]:
        ...
        _last_mid   = float(_ph["price"].iloc[-1])
        _y_min      = float(_ph["price"].min())
        _y_max      = float(_ph["price"].max())
        _pad        = max((_y_max - _y_min) * 0.3, 0.00005)  # at least 0.5 pip padding

        # Split REST-polled vs real stream ticks for visual distinction
        _ph_rest   = _ph[_ph.get("stream", pd.Series([False]*len(_ph))) == False] if "stream" in _ph.columns else _ph
        _ph_stream = _ph[_ph["stream"] == True] if "stream" in _ph.columns else pd.DataFrame()

        _fig = go.Figure()
        # Continuous price line (all points)
        _fig.add_trace(go.Scatter(
            x=_ph["time"], y=_ph["price"],
            mode="lines",
            name="Mid (REST poll)",
            line=dict(color="#4DA6FF", width=1.8),
        ))
        # Overlay real stream ticks as dots
        if not _ph_stream.empty:
            _fig.add_trace(go.Scatter(
                x=_ph_stream["time"], y=_ph_stream["price"],
                mode="markers",
                name="Stream tick",
                marker=dict(symbol="circle", size=5, color="#00E5FF",
    ...
    with _c1:
        if _app["tick_log"]:
            ...
            st.markdown(
                ...
                f'border:1px solid #30363d;border-radius:6px;padding:10px 14px;'
                f'font-family:monospace;font-size:12px;color:#cdd9e5;line-height:1.7;">'
                f'{_tick_html}</div>',
                unsafe_allow_html=True,
            )
        else:
